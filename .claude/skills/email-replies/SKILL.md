...
the project's `PERSONALITY.md` first — that is the source of truth for tone. The
reply must still land the real point, not just match the vibe. If a reply needs a
fact you don't have, leave a marked placeholder like `[confirm the date]` — never
invent.
...
> Drafted 4 replies → drafts/replies-2026-06-21.md

If there were no unread emails worth replying to, say so plainly and don't create
an empty file.
